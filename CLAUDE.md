...
verify-at-send gate but it is untested at volume, and the SMTP verifier timed out
on every call in the last session.

On that last point there is now a known mechanism and a fix, but it is **not a
proven diagnosis** — see `fetchT` in PART 6. The verifier failing because Render
cannot reach `client.myemailverifier.com` at all would look identical from the
outside. The next live run will say which it is, because the error branch now
names DNS / refused / TLS / genuine-timeout separately instead of printing one
word. Vin has a second domain available and is not yet rotating.
