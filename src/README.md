...
- **The manifest is the order.** `manifest.js` is the program's statement order (today's, interleaved across departments). Several boot checks assert byte order across it (`ACCESS CHECK`: the body parser before `serverKeyGate`) and 48 count occurrences across the whole built file, so reordering it is a code change with its own proof, never a tidy-up.
...
A part of a cut function adds `// PART n OF N of <function>: does not parse alone; joined in manifest order.` Every file: LF, no BOM, non-empty, ends with exactly one newline, at most about 800 lines, one job. A header may not contain `✓`, `⛔`, `process.env.`, `req.query.`, `readFileSync(`, `__filename` or `Hold();` (the counting checks and `gen-refs` scan comments too). No "last changed" line; `git log` is the record. Shape: the header, ONE blank line, then the body. The header ends at the first blank line or the first non-comment line; a file never ends on a blank line (the build refuses it: that blank line is a line of `server.js`, so it moves to the top of the NEXT file, just after its header, never deleted). If the build refuses a forbidden string on a line that is the program's own comment rather than the header, put one blank line after the header's last line so the header ends there.

What `build.js` actually refuses, naming file:line (a manifest refusal names `src/manifest.js` and the entry; its own header is the authority): a CR byte, a BOM, an empty file, a missing or doubled final newline, a line that is not valid UTF-8 or carries a literal U+FFFD, a manifest that does not load or lists a path that is not plain and src-relative, an entry not on disk, a duplicate entry, a `src/**/*.js` the manifest does not list, a symbolic link under `src/`, an entry that is not a regular file (a directory), a file that ends on a blank line, a file over the cap (once the manifest lists more than one file), a file with no header comment, a header with no `Goal:` line (one starting `// Goal: after reading this file, Claude can`), a first manifest file whose first line is not the exact GENERATED banner, and a forbidden string in a header. The department line and the Owns / Defines for others / Guarded by lines are the house shape, read by a person and by the header-verification agents at the cut — not by the build. A header is a claim; write it from what the code does.
