---
name: ship-round
description: "L4 JOBS: Close a round the way this repo does: write docs/history/round-NNN.md in the house shape, add its INDEX.md line, bump CONTRACT_VERSION (in src/, rebuilt into server.js) and CLIENT_CONTRACT (index.html) together whenever index.html changed, shape the commit message and the PR, and hand over what needs hands (Netlify drag-in, Supabase SQL, Render env vars, credit top-ups). Use when a change is ready to merge or the user says ship, close the round, write the round note."
...
`bash verify.sh` green (the byte proof, the static stage, one boot), then every new guard falsified with `node falsify.js docs/history/round-NNN-reverts.js` — the round's reverts live in that file, committed beside the note, so the proof can be rerun (`falsify` note) — then `bash ci-gates.sh` all green (`gates` note). A push that turns CI red costs a cycle and trust.
...
- Voice: what Vin reported (his words, quoted), what was found — reproduced by execution, each defect in terms of what it did to a lead — what changed and why at the root, "what the falsification runs found in the checks themselves" (one line per revert in `docs/history/round-NNN-reverts.js` — name, verdict, the named red line — plus any falsification run by hand, recorded separately), the boot-check count, the line stating whether `index.html` changed and therefore needs the Netlify drag-in (every round since [§25](../../../docs/history/round-025.md) carries it), and — since [§128](../../../docs/history/round-128.md) — a fixed last section, **"Needs your eyes"**: only what a machine cannot prove (the merge, the Netlify drag, Render variables and Supabase SQL, anything a rep or a prospect sees, spend, whether the names read right), each with what to look at and what good looks like. Everything else is reported as proven, with the command that proved it.
...
Say plainly, in this order: whether Render redeploys (server.js changed?), whether Netlify needs the drag-in (index.html changed?), any SQL to run in Supabase, any env var to set on Render, any account to top up (Firecrawl, Apify, DataForSEO, the email verifier, Anthropic), and what the first live run should be grepped for to prove the round (`OWNER WAVE`, `FIND YIELD`, `LOCAL PACK`, …). A stale page will say so by contract number; say that too. Then end on **Needs your eyes** — the note's fixed last section (above): only the validation zones, nothing a command already proved.
