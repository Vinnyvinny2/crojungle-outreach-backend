...
- [ ] **Render**: does `src/` change (so the built `server.js`)? If yes, it redeploys on merge to `main`; the health check at `/healthz` holds a red boot back (PART 8 step 1), and the boot log must print `✓ BUILD CHECK`.
...
- [ ] **Needs your eyes** — the note's fixed last section ([§128](../../../docs/history/round-128.md)): only the merge, the Netlify drag, Render variables and Supabase SQL, anything a rep or a prospect sees, spend, and whether the names read right; each with what to look at and what good looks like. Everything else is reported as proven, with its command.
