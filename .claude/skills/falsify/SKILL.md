...
2. **Revert ONE fix, verbatim.** Restore the SHIPPED expression exactly as it was, from a byte probe of the live file — not "a line near it", not a design note's memory of it ([§77](../../../docs/history/round-077.md), [§87](../../../docs/history/round-087.md)). Keep every byte as it was: LF under `src/` (the build makes the CRLF — `server.js` is generated, and is rebuilt after a source revert is applied and again after it is restored), CRLF in `server.js` only when the revert IS the hand edit; the harness once flattened 64,887 line endings and made every run red ([§74](../../../docs/history/round-074.md)).
...
- The harness is `falsify.js` at the repo root, since [§128](../../../docs/history/round-128.md); a round's reverts are a plain data module, `docs/history/round-NNN-reverts.js` (the header of `falsify.js` is the contract for its shapes), committed beside the round note so the proof can be rerun: `node falsify.js docs/history/round-NNN-reverts.js`. It proves the baseline green first (boot, clientcheck, `build-check`, and every proof the list uses, stopping at the first red kind so a hand-edited `server.js` is never rebuilt over before the run stops — a stale `server.js` stops here as BASELINE NOT GREEN), rebuilds `server.js` after applying a revert under `src/` and again after restoring it, and VERIFIES every restore instead of trusting it: a sha1 of `server.js` and every file under `src/` plus `git status --porcelain`, taken once the baseline is green and compared after every revert and at the end — any difference, an undo that exits non-zero, or a rebuild that refuses prints `RESTORE FAILED` naming it and the run exits 1 whatever the verdicts. Every revert carries `mustPrint` (a string or a RegExp): the guard's OWN line, which the red run must have printed, or it is reported "RED but NOT on the guard's own line" and does not match. A selection that names no revert (an empty list, a mistyped name) exits 2 before the baseline — "0 of 0 matched" was once a green exit.
