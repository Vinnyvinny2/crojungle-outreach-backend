...
The full list of every setting the server reads (81 names, first line, parsed default) is generated into `env-vars.md` beside this note; the table below is the subset that matters day to day.

Copied from CLAUDE.md (commit b01d952) lines 10624 to 10646 (PART 8, the knobs table), with one default corrected in place on 2026-09-02 (listed at the end). The other defaults were checked against the code and match: FC_DAILY_BUDGET 1500, PLACES_DAILY_BUDGET 600, ANTHROPIC_DAILY_BUDGET_USD 20, APIFY_DAILY_BUDGET 150, APIFY_MAX_REVIEWS 90, SITUATION_EFFORT high, FC_CONCURRENCY 2, RESEARCH_CONCURRENCY 8, FIND_RUN_MAX 300. The client handshake (CONTRACT_VERSION / CLIENT_CONTRACT) lives in the `ship-round` skill; the one-time Render / GitHub / Netlify steps live in `deploy-and-accounts`. Where every API KEY comes from is declared in code as `KEY_SOURCES` in server.js and checked at boot — search for it rather than guessing.
...
| `REVIEW_CORPUS_CHARS` | 36000 | how many of them the pain miner actually reads. Raise the pull without raising this and you are paying for reviews no model sees — see §54 |
...
| `APP_TOKEN` | unset (gate OPEN, loud) | the one shared access code. Set, every `/api` route but the cron answers 401 without `Authorization: Bearer <code>`; unset, the boot prints `AUTH GATE OFF` and `/healthz` says `auth: off`. Set it once the §127 page is on Netlify; on a Render instance an unset code is a red `ACCESS CHECK` ([§126](../../../docs/history/round-126.md), [§127](../../../docs/history/round-127.md)) |
| `ALLOWED_ORIGINS` | unset (every origin) | comma-separated browser origins the server answers (`https://<netlify host>`). Empty keeps the old open door; a listed origin is echoed; anything else gets no CORS header and a 403 preflight ([§126](../../../docs/history/round-126.md)) |
| `ANTHROPIC_API_KEY` / `FIRECRAWL_KEY` / `APIFY_TOKEN` / `HUNTER_KEY` / `MYEMAILVERIFIER_KEY` / `COMPANIES_API_KEY` / `THEIRSTACK_KEY` / `PDL_KEY` / `NINJAPEAR_KEY` / `FB_TOKEN` / `ADZUNA_ID` / `ADZUNA_KEY` | unset | the paid keys, on Render since [§126](../../../docs/history/round-126.md) (`SERVER_KEY_ENV`). A set one replaces whatever the page sends on every `/api` request and feeds the background read runs; an unset one turns that source off and the route refuses by name (since §127 a key in a request or in the Settings row is ignored). `/api/store/settings` reports which are held, as booleans; the Settings screen shows the list |
