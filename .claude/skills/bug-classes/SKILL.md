...
**CRLF.** `server.js` is CRLF on all of its about 84,000 lines, and since [§128](../../../docs/history/round-128.md) that CRLF is PRODUCED by `build.js` from LF sources under `src/`; `index.html`, `src/` and every check script are LF. A tool that normalises newlines rewrites the whole file: Python's default `open()` flattened all 64,887 endings once and turned two CRLF-sensitive checks red on every run ([§74](../../../docs/history/round-074.md)); a replacement authored in a Python triple-quoted string put 83 LF lines into the file and still passed `node --check` ([§89](../../../docs/history/round-089.md)); a needle spanning two lines with `\r\n` can never match the LF-normalised source copy ([§103](../../../docs/history/round-103.md)). Rule: never edit `server.js`; after any edit under `src/`, `node build.js --check` must exit 0 — it proves the built file is byte for byte what the sources build, with every built line ending in CRLF (so `grep -c $'\r' server.js` still equals `wc -l < server.js`), and `build.js` refuses a CR byte in any source file, naming file:line; see the `editing-server-js` skill.
