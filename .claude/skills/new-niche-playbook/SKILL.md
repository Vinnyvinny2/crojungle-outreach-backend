...
New text (2026-09-02; the source layout 2026-09-09, [§128](../../../docs/history/round-128.md)), from a line-by-line survey of the declared tables in `server.js` at commit b01d952 (`tables.md` beside this note has every one with its source file and line, its built line and its check). **Filling these tables is an edit under `src/` — rebuilt into `server.js` by `node build.js`, proven by `node build.js --check` — and every rule in `editing-server-js` applies** (LF source, needles, the gates); never a hand edit of the generated `server.js`. The honest end state is one data file per niche so this playbook becomes a form; that extraction moves code under `src/` and is a later round. Today the tables are inside the source (`src/`), and the boot is the form: it refuses to start until each declared table has its row, and says which.
